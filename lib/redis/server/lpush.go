...
func (c lpush) execute(ctx context.Context, params commandParams) commandResult {
	if len(params) < 3 {
		return resptypes.Error{Val: fmt.Errorf("ERR LPUSH requires key and at least one element! %s", c.getUsage(ctx))}
	}
	...
	return resptypes.Error{Val: fmt.Errorf("ERR LPUSH can only be called on lists! %s", c.getUsage(ctx))}
}
